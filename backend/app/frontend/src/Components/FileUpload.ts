...
interface FileUploadOptions {
  onProcessed: (analysisId: number) => void;
}
...
export class FileUpload {
  ...
  render(): HTMLElement {
    ...
    wrapper.append(
      createElement("div", "upload-header", [
  ...
  private setSteps(status: ("pending" | "active" | "done" | "error")[]): void {
    this.stepsContainer.replaceWith(this.stepsContainer = ProcessingSteps(status));
  }

  private resetButtons(disabled: boolean): void {
  ...
  private async process(): Promise<void> {
    ...
    try {
      const response = await fetchJson<{ analysis_id: number; confidence?: number; processing_ms?: number }>(
        "/audio/process_full",
